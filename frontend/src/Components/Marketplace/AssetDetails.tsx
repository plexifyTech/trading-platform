...
import AlertDialog from "../Common/MessagePopup";
...
const AssetDetails = (asset: AssetFields) => {
    ...
    const [availableAssets, setAvailableAssets] = React.useState<number>(asset.details.availableAssets);
    ...
    return (
        ...
        >
            ...
            <Box sx={{flexGrow: 1, backgroundColor: "#191b21", padding: "2rem"}} >
                <SparkLineChart data={asset.details.prices} height={100} colors={["#6c8da3"]}/>
            </Box>
            <FixedGrid>
                ...
                <div>
                    <Button onClick={handleBuy} disabled={assetUnavailable()} variant="contained" sx={{width: "150px"}}>BUY</Button>
                </div>
            </FixedGrid>
        </Stack>
        </>
